...
function adaptationLogic(_mpd, video)
{
	...
	this.length = _mpd.period[0].group[0].representation.length;
	
	this.representationID = m;
	this.lowestRepresentationID = m;
	this.highestRepresentationID = this.length - 1;
	...
	this.highestRepresentation = _mpd.period[0].group[0].representation[this.length - 1];
	...
	if(this.highestRepresentation.baseURL == false) this.highestRepresentation.baseURL = _mpd.baseURL;
...
function init_rateBasedAdaptation(_mpd, video,bandwidth)
{
	...
	rateBasedAdaptation.prototype.switchLowerRepresentation = function (){
			// select a matching bandwidth ...
			var m=this.representationID;
			
			if(m != this.lowestRepresentationID){
				this.representationID = m - 1;
				this.mpd.period[0].group[0].representation[m-1].curSegment = this.currentRepresentation.curSegment;
				this.currentRepresentation = this.mpd.period[0].group[0].representation[m-1];
				if(this.currentRepresentation.baseURL == false) this.currentRepresentation.baseURL = _mpd.baseURL;
			}
			this.notify();
		}
		
	rateBasedAdaptation.prototype.switchHigherRepresentation = function (){
			// select a matching bandwidth ...
			var m=this.representationID;
			
			if(m != this.highestRepresentationID){
				this.representationID = m + 1;
				this.mpd.period[0].group[0].representation[m+1].curSegment = this.currentRepresentation.curSegment;
				this.currentRepresentation = this.mpd.period[0].group[0].representation[m+1];
				if(this.currentRepresentation.baseURL == false) this.currentRepresentation.baseURL = _mpd.baseURL;
